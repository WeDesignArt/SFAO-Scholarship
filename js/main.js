jQuery(document).ready(function ($) {
  ...
  // faq
  // FAQ accordion: only one open at a time; toggle `active` on header and `current` on item
  (function () {
    const $faqBtns = $(".btn_faq");

    // Initialize: hide all contents, show the one marked active and mark its parent
    $(".faq_single_content").hide();
    $faqBtns.each(function () {
      const $btn = $(this);
      const $content = $btn.next(".faq_single_content");
      const $item = $btn.closest(".faq_single");
    ...
    $faqBtns.on("click", function () {
      ...
      // Close all
      $faqBtns.removeClass("active");
      $(".faq_single_content").not($content).slideUp(250);
      $(".faq_single").removeClass("current");
      ...
      if (!isActive) {
        ...
        // It was active — now closed (already closed by slideUp)
  ...
  const scholarshipSwiper = document.querySelector(".scholarship_swiper");
  if (scholarshipSwiper) {
    new Swiper(scholarshipSwiper, {
      grabCursor: true,
      autoplay: {
        delay: 2500,
      },
      loop: true,
      pagination: {
        el: ".scholarship_pagination",
        clickable: true,
        renderBullet: function (index, className) {
          return (
            '<span class="bullet_point ' +
            className +
            '">' +
            (index + 1) +
            "</span>"
      ...
      navigation: {
        nextEl: ".scholarship_next",
        prevEl: ".scholarship_prev",
      },
      breakpoints: {
        // when window width is >= 320px
        320: {
  ...
  const storieSwiper = document.querySelector(".stories_swiper");
  if (storieSwiper) {
    new Swiper(storieSwiper, {
      grabCursor: true,
      loop: false,
      pagination: {
...
// Dropdown selection (scholarship page-UOK)
 ...
 document.querySelectorAll('.dropdown-item').forEach(item => {
      item.addEventListener('click', function (e) {
        ...
        dropdown.querySelector('.scholarship-text').textContent =
...
// Custom Accordion Button Behavior (scholarship page-UOK)
     document.querySelectorAll('.custom-accordion-btn').forEach(btn => {
